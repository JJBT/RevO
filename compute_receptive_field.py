    ...
    def show(self, image=None, axes=None, show=True):
        ...
        for (index, (x, y, w, h)) in enumerate(rects):  # iterate RFs
            ...
            if index == 0 or index == len(rects) - 1 or t.rand(1).item() < rect_density:
                axes.add_patch(
                    patches.Rectangle((x, y), w, h, facecolor=marker.get_color(), edgecolor='none', alpha=0.5))
                first = False
...
def receptivefield(net, input_shape, device='cpu'):
    """Computes the receptive fields for the given network (nn.Module) and input img_size, given as a tuple (images, channels, height, width).
    Returns a ReceptiveField object."""

    if len(input_shape) < 4:
        raise ValueError('Input img_size must be at least 4-dimensional (N x C x H x W).')
...
def decrease_stride(model, layers, prefix=None):
    if prefix is not None:
        layers = ['.'.join([prefix, layer]) for layer in layers]
...
net = MyResNet()
decrease_stride(net, layers=['layer2', 'layer3', 'layer4'], prefix='resnet')
# inp = torch.zeros((1, 3, 320, 320))
# print(f'Model stride: {320 / net(inp).img_size[2]}')

rf = receptivefield(net, (1, 3, 512, 512))
print(rf)
# rf.show()
