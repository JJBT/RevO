"""Make an artificial large image dataset using Omniglot characters"""
...
class MegapixelOmniglot:
    ...
    def __init__(self, root, N=5000, W=1500, H=1500, train=True, cat_ids=None, is_colourful=False, affine=False, seed=42):
        ...
        omniglot = Omniglot(root=root, background=train, download=download, transform=lambda x: 255 - np.array(x))

        if cat_ids is None:
            cat_ids = []

        data = list(map(list, zip(*[elem for elem in omniglot])))
        x = np.array(data[0])
        y = np.array(data[1])
        ...
        self.cats_dict = [{'id': i, 'name': label} for i, label in enumerate(omniglot._characters)]
...
def main(omniglot_path, megapixel_omniglot_path):
    ...
    # val_train_cats = MegapixelOmniglot(
    #     root=omniglot_path,
    #     N=n_train_val,
    #     train=True,
    #     cat_ids=train_cat_ids,
    ...
    val_novel_cats = MegapixelOmniglot(
        root=omniglot_path,
        N=n_val,
        train=False,
...
if __name__ == "__main__":
    omniglot_path = '/home/vladimir/data/omniglot/'
    megapixel_omniglot_path = '/home/vladimir/data/megapixel_omniglot/'
